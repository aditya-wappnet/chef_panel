...
String getTimeAgo(String dateString, {bool numericDates = true}) {
  ...
  if (difference.inSeconds < 5) {
    ...
    return (numericDates)
        ? AppLocalizations.of(context!).translate('1_minute_ago')
        : AppLocalizations.of(context!).translate('a_minute_ago');
  } else if (difference.inMinutes <= 60) {
    return '${difference.inMinutes} ${AppLocalizations.of(context!).translate('minutes_ago')}';
  } else if (difference.inHours <= 1) {
    return (numericDates)
        ? AppLocalizations.of(context!).translate('1_hour_ago')
        : AppLocalizations.of(context!).translate('an_hour_ago');
    ...
    return (numericDates)
        ? AppLocalizations.of(context!).translate('1_week_ago')
        : AppLocalizations.of(context!).translate('last_week');
  } else if ((difference.inDays / 7).ceil() <= 4) {
    return '${(difference.inDays / 7).ceil()} ${AppLocalizations.of(context!).translate('weeks_ago')}';
  } else if ((difference.inDays / 30).ceil() <= 1) {
    return (numericDates)
        ? AppLocalizations.of(context!).translate('1_month_ago')
    ...
    return (numericDates)
        ? AppLocalizations.of(context!).translate('1_year_ago')
